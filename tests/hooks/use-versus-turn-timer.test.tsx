...
describe('useVersusTurnTimer', () => {
  ...
  it('resets the online deadline on turn change even when the shared timer ref is already on the new player', () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-03-29T20:00:00.000Z'))

    try {
      const activeTurnTimerKeyRef = { current: 'versus-puzzle:o' as string | null }
      const setTurnTimeLeft = vi.fn()
      const setTurnDeadlineAt = vi.fn()
